...
class DefaultTeamPlayerTableAccess(AbstractTableAccess[DefaultTeamPlayer]):
    ...
    async def get_all_async(self) -> list[DefaultTeamPlayer]:
        r = await self._database.execute_query_async(
            "SELECT * FROM DefaultTeamPlayer"
            )
        return [DefaultTeamPlayer(id=row[0], player=row[1], team=row[2])
                for row in r]
...
class PlayerPenaltiesTableAccess(AbstractTableAccess[PlayerPenalties]):
    ...
    async def get_by_gameplayerid_async(self, gameplayerid: int) -> list[PlayerPenalties]:
        query = "SELECT * FROM PlayerPenalties WHERE GamePlayer = ?"
        params = (gameplayerid,)
...
class TeamPenaltiesTableAccess(AbstractTableAccess[TeamPenalties]):
    def get_all(self) -> list[TeamPenalties]:
        r = self._database.execute_query("SELECT * FROM PenaltyKind")
        return [
            TeamPenalties(id=row[0], team=row[1], penalty=row[2])
            for row in r
        ]
...
class ResultOfGameViewAccess(AbstractDatabaseAccess[ResultOfGame]):
    ...
    def get_by_game_id(self, game_id: int) -> ResultOfGame:
        ...
        r = self._database.execute_single_query(
            "SELECT * FROM ResultOfGame WHERE Id = ?", params
        )
...
class SumPerGameViewAccess(
    AbstractTableAccess[SumPerGame]
):
    ...
    def get_by_game_id(self, game_id: int) -> SumPerGame:
        params = (game_id,)
        r = self._database.execute_single_query(
            "SELECT * FROM SumPerGame WHERE GameId = ?", params
        )
